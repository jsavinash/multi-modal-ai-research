...
4. Build `tests/fixtures/` — at least 20 small real files including the hostile cases: zero-byte file;
...
5. Text + structured + code extractors with tests. **Commit.**
6. Web + PDF + office + archive extractors with tests, including the nested-zip case and a synthetic
   zip bomb config refuses. **Commit.**
7. `test_registry_coverage.py` and `test_pipeline.py` (whole-fixtures ingest, zero exceptions).
8. Run the gate; measure peak RSS.
...
DELIVERABLES - exactly these paths
  ...
  tests/fixtures/                     (>= 20 real files - see task 4)
  tests/test_ir.py
  tests/test_extractor_contract.py
  tests/test_extractors_{text,structured,code,web,pdf,office}.py
  tests/test_archive.py
...
TASKS
  ...
  4. Build tests/fixtures/ with at least 20 small real files. MUST include the hostile cases:
  ...
  5. text/structured/code extractors plus tests. COMMIT.
  6. web/pdf/office/archive extractors plus tests, including the nested-zip case and a
